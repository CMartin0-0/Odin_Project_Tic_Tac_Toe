...
const game = (() => {
    ...
    const handleRound = () => {
        ...
        let emptyCells = gameBoard.getBoard().filter(board => board.value == "");
    ...
    const checkForWin = () => {
        let oCells = gameBoard.getBoard().filter(board => board.value === "o");
        let xCells = gameBoard.getBoard().filter(board => board.value === "x");
        const winCombinations = [[0, 1, 2], [0, 3, 6], [0, 4, 8], [1, 4, 7], [2, 4, 6], [2, 5, 8], [3, 4, 5], [6, 7, 8]];
